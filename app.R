...
srv_nuts1 <- readRDS("data/srv_nuts0.rds")
srv_nuts2 <- readRDS("data/srv_nuts0.rds")
...
titles <- categories %>%
  map(~filter(cb_ext, topic == .x) %>% pull(title) %>% unique() %>% as.list()) %>%
  ...
  fluidRow(
    bs4Dash::column(
      ...
      bs4Dash::box(
        ...
        shinyjs::hidden(
          div(id = "subitem_hide",
            pickerInput("exp_subitem", "Subitem", character())
          )
        ),
        shinyjs::hidden(
          div(id = "option_hide",
            pickerInput("exp_option", "Option", character())
          )
        )
      ),
      bs4Dash::box(
        ...
        collapsible = TRUE,
        status = "primary",
        pickerInput("scale", "Aggregation level", c("NUTS-0", "NUTS-1", "NUTS-2")),
        pickerInput("pal", "Color palette", all_pals)
      ),
      bs4Dash::box(
        title = "Download",
        id = "exp_download",
  ...
  fluidRow(
   ...
   column(
     ...
     bs4Dash::box(
       ...
       width = 12,
       status = "primary",
       fluidRow(column(width = 3.5,
           pickerInput("cs1bounds",
             label = "Boundaries",
             choices = c("Quarters", "Zones", "Statistical areas"),
             pickerOptions(windowPadding = c(0, 0, 1000, 0))
           )
         ),
  ...
  dashboardHeader(
    ...
    tagList(
      a(
        ...
        img(src = "gesis_logo.png")
      ),
      a(
        class = "logo",
        href = "https://www.unibo.it/",
        img(src = "unibo_logo.png")
      ),
      a(
        class = "logo",
        href = "https://www.tecnalia.com/",
        img(src = "tecnalia_logo.png")
      )
    ),
    title = HTML(paste(
      img(src = "https://projectgreta.eu/wp-content/uploads/2021/09/salama.svg", width = 50, height = 50),
...
server = function(input, output, session) {
  ...
  observeEvent(input$exp_title, {
    ...
    if (show_subitems) {
      updatePickerInput(session, inputId = "exp_subitem", choices = items)
    ...
    if (show_options) {
      updatePickerInput(session, inputId = "exp_option", choices = options)
